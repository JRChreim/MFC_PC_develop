# 1 "/ocean/projects/phy230019p/jrchreim/MFC-JRChreim/src/post_process/m_data_output.fpp"
!>
...
# 642 "/ocean/projects/phy230019p/jrchreim/MFC-JRChreim/src/post_process/m_data_output.fpp"
            if (precision == 1) then
                if (p > 0) then
                    err = DBMKOPTLIST(2, optlist)
                    ...
                    if (grid_geometry == 3) then
                        err = DBPUTQM(dbfile, 'rectilinear_grid', 16, &
                                        'x', 1, 'y', 1, 'z', 1, &
                                        y_cb_s, z_cb_s, x_cb_s, dims, 3, &
                                        DB_FLOAT, DB_COLLINEAR, &
                                        optlist, ierr)
                    else
                        err = DBPUTQM(dbfile, 'rectilinear_grid', 16, &
                                        'x', 1, 'y', 1, 'z', 1, &
                                        x_cb_s, y_cb_s, z_cb_s, dims, 3, &
                                        DB_FLOAT, DB_COLLINEAR, &
                                        optlist, ierr)
                    end if
                    err = DBFREEOPTLIST(optlist)
                    ...
                    err = DBPUTQM(dbfile, 'rectilinear_grid', 16, &
                                'x', 1, 'y', 1, 'z', 1, &
                                x_cb_s, y_cb_s, DB_F77NULL, dims, 2, &
                                DB_FLOAT, DB_COLLINEAR, &
                                optlist, ierr)
                    err = DBFREEOPTLIST(optlist)
                end if
...
# 674 "/ocean/projects/phy230019p/jrchreim/MFC-JRChreim/src/post_process/m_data_output.fpp"
...
# 871 "/ocean/projects/phy230019p/jrchreim/MFC-JRChreim/src/post_process/m_data_output.fpp"
                if (precision == 1) then
                    if (p > 0) then
                        if (grid_geometry == 3) then
                            err = DBPUTQV1(dbfile, trim(varname), &
                                            len_trim(varname), &
                                            'rectilinear_grid', 16, &
                                            cyl_q_sf_s, dims - 1, 3, DB_F77NULL, &
                                            0, DB_FLOAT, DB_ZONECENT, &
                                            DB_F77NULL, ierr)
                        else
                            err = DBPUTQV1(dbfile, trim(varname), &
                                            len_trim(varname), &
                                            'rectilinear_grid', 16, &
                                            q_sf_s, dims - 1, 3, DB_F77NULL, &
                                            0, DB_FLOAT, DB_ZONECENT, &
                                            DB_F77NULL, ierr)
                        end if
                    else
...
# 898 "/ocean/projects/phy230019p/jrchreim/MFC-JRChreim/src/post_process/m_data_output.fpp"
