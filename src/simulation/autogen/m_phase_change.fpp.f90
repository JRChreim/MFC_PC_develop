# 1 "/ocean/projects/phy230019p/jrchreim/MFC-JRChreim/src/common/m_phase_change.fpp"
!>
...
# 6 "/ocean/projects/phy230019p/jrchreim/MFC-JRChreim/src/common/m_phase_change.fpp" 2
...
module m_phase_change
    ...
    !> @{
    integer, parameter :: max_iter = 1e8        !< p_relaxk \alpha iter,                set to 25
    ! real(kind(0d0)), parameter :: pCr = 22.09d6   !< Critical water pressure              set to 22.06d6
    ! real(kind(0d0)), parameter :: pCr = 1.82d6   !< Critical water pressure              set to 22.06d6
    real(kind(0d0)), parameter :: pCr = 4.94d7   !< Critical water pressure              set to 22.06d6
    ! real(kind(0d0)), parameter :: TCr = 647.29d0  !< Critical water temperature           set to 648
    real(kind(0d0)), parameter :: TCr = 385.05+273.15  !< Critical water temperature           set to 648
...
!$acc loop seq                    
                    ...
                    if ((relax_model == 6) .and. ((q_cons_vf(lp + advxb - 1)%sf(j, k, l) > palpha_eps)  &
                        ...
                        ! PRINT *, q_cons_vf(lp + contxb - 1)%sf(j, k, l)
                        ! PRINT *, q_cons_vf(vp + contxb - 1)%sf(j, k, l)
                        ! PRINT *, mixM*rM
...
&                    . Please, check energy constraint. Aborting!')
        ...
        do while ((DABS(pS - pO) > ptgalpha_eps) .and. (DABS((pS - pO)/pO) > ptgalpha_eps/1e4) .or. (ns == 0))
...
&                    Please, check the pressure value, or the maximum number of iteractions. Aborting!')
        ...
        ! PRINT *, q_cons_vf(lp + contxb - 1)%sf(j, k, l), q_cons_vf(lp + advxb - 1)%sf(j, k, l)
...
!$acc routine seq
    ...
    ! subroutine s_state_variables(DeltamP, InvJac, j, Jac, k, l, mQ, p_infA, pS, R2D, rhoe, q_cons_vf, TS) ! ----------------

    ! end subroutine s_state_variables
...
&                            (m_phase_change, s_TSat). Aborting!')
        ...
        ! PRINT *, 'p', pSat, 'T', TSat, 'FT', FT, 'eps', ptgalpha_eps
        ! PRINT *, 'FT calc', A + B / TSat + C * DLOG( TSat ) + D * DLOG( ( pSat + ps_inf( lp ) ) ) - DLOG( pSat + ps_inf( vp ) )
        ! PRINT *, 'ns', ns
        ! PRINT *, A, B, C, D
